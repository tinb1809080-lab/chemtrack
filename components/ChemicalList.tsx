...
const ChemicalList: React.FC<ChemicalListProps> = ({ chemicals, auditLogs, userRole, onEdit, onDelete, onLotAction, onAddLot }) => {
  ...
  return (
    <div className="space-y-4">
      {chemicals.map(chem => {
        const totalStock = chem.lots
          .filter(l => l.status === 'RESERVED' || l.status === 'IN_USE')
          .reduce((sum, l) => sum + l.quantity, 0);
        const isLowStock = totalStock <= (chem.minThreshold || 0);

        return (
          <div key={chem.id} className={`bg-white rounded-xl shadow-sm border overflow-hidden transition-all hover:border-indigo-200 ${isLowStock ? 'border-amber-200' : 'border-gray-100'}`}>
            <div 
              className={`p-4 flex items-center justify-between cursor-pointer hover:bg-gray-50 transition-colors ${isLowStock ? 'bg-amber-50/20' : ''}`}
              onClick={() => {
            ...
            >
              <div className="flex items-center space-x-4">
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    ...
                    {isLowStock && (
                      <span className="bg-amber-100 text-amber-700 text-[9px] font-black px-2 py-0.5 rounded-full flex items-center gap-1 border border-amber-200 animate-pulse">
              ...
              <div className="flex items-center space-x-6">
                <div className="text-right">
                  ...
                  <p className={`font-black text-xl ${isLowStock ? 'text-amber-600' : 'text-indigo-600'}`}>
                    {totalStock.toFixed(2)} <span className="text-xs text-gray-400 font-normal">{chem.lots[0]?.unit || ''}</span>
                  </p>
                </div>
            ...
            {expandedChem === chem.id && (
              <div className="bg-slate-50 border-t border-gray-100 p-5">
                              ...
                              {showingHistory && (
                                <tr>
                                  <td colSpan={5} className="bg-indigo-50/30 px-5 py-4 animate-in slide-in-from-top-1">
                                    {/* History Table UI - Same as before */}
                ...
                <div className="mt-6 pt-4 border-t border-slate-200 flex justify-between items-center text-[11px]">
                  <div className="text-slate-400 font-bold uppercase tracking-widest flex items-center gap-4">
                    <div>Vị trí: <span className="text-slate-700">{chem.location}</span></div>
                    <div>Ngưỡng báo động: <span className="text-amber-600 font-black">{chem.minThreshold} {chem.lots[0]?.unit || ''}</span></div>
                  </div>
